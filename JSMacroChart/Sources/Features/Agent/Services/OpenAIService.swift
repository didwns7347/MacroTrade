...
enum APIKeyManager {
    static func getAPIKey() -> String {
        ...
        guard let apiKey = Bundle.main.object(forInfoDictionaryKey: "OPEN_API_KEY") as? String,
              !apiKey.isEmpty,
              !apiKey.starts(with: "$(") else {
            fatalError("Info.plist에 `OPEN_API_KEY`를 설정해야 합니다. Config.xcconfig의 값이 올바르게 반영되었는지 확인하세요.")
        }
        return apiKey
    }
}
